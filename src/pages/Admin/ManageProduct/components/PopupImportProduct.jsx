            ...
            for (const batch of batches) {
                const promises = batch.map(async (product) => {
                    try {
                        ...
                        const sizes = typeof product.sizes === 'string' ? product.sizes.split(',') : [product.sizes]
                        ...
                        if (product.productImage) {
                            ...
                            const productImage = typeof product.productImage === 'string'
                            ...
                            const promises = productImage.map(async (url) => {
                                try {
                                    ...
                                    const placeholder = dataURItoBlob('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==');
                            ...
                            colors.forEach(() => {
                                const blob = dataURItoBlob('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==');
        ...
        <Modal open={isModalOpen} onClose={handleClose}>
            ...
            >
                <h2 className='text-3xl font-bold text-center uppercase text-green-700 mb-5'>
                    {uploadStep === 1 ? 'Nhập Sản Phẩm Từ File' :
                        uploadStep === 2 ? 'Xem Trước Dữ Liệu' : 'Xác Nhận Nhập Dữ Liệu'}
                </h2>
                    ...
                    <div>
                        <div className='mb-4'>
                            <Typography variant='body1' gutterBottom>
                                Xem trước {previewData.length} dòng đầu tiên trên tổng số {importData.length} sản phẩm:
                            </Typography>
                        </div>
                        ...
                        <Box sx={{ bgcolor: '#f8f9fa', p: 2, borderRadius: 1, mb: 4 }}>
                            <Typography variant='body2' color='text.secondary'>
                                <strong>Lưu ý:</strong> Khi nhập sản phẩm từ file, các sản phẩm sẽ được thêm vào không có hình ảnh.
                                Bạn cần cập nhật hình ảnh sản phẩm sau khi nhập.
                            </Typography>
                        </Box>
